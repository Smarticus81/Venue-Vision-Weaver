...
- **`artifacts/api-server`** - Express 5 backend. Serves the wedding-app SPA as static files. Routes in `src/routes/` (venues, sessions, storage, billing, gallery styles, control plane). Credit-gated Gemini image generation with Stripe billing.
